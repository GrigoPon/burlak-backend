...
3. **`processing_cards`**: Параллельная обработка каждой карты: парсинг $\rightarrow$ перевод через ML $\rightarrow$ инкремент прогресса в БД.
...
5. **Работа с ML и XLSX:**
    * Для отправки данных в ML-сервис использовать конвертацию `xlsx` $\rightarrow$ `json`.
