...
def generate_discrepancy_report(
    ...
    ws_summary.merge_range(
        "A2:H2",
        f"Проверено {result.total_configs} комплектаций | Деталей в BOM: {result.total_bom_unique_parts} | В картах: {result.total_cards_unique_parts}",
        subtitle_fmt,
    )
    ...
    if cards_data:
        corrupted_count = (
            len(cards_data.corrupted_files) if cards_data.corrupted_files else 0
        )
    ...
    ws_summary.merge_range(
        info_row + 1, 0, info_row + 1, 7, "СВОДКА ПО КОМПЛЕКТАЦИЯМ", table_title_fmt
    )
    ...
    for ri, cr in enumerate(result.config_results, config_header_row + 1):
        short = (
            cr.config_name if len(cr.config_name) <= 52 else cr.config_name[:49] + "..."
        )
    ...
    if fuzzy_discs:
        ...
        for ri, disc in enumerate(fuzzy_discs, 1):
            ...
            ws_fuzzy.write(
                ri, 4, disc.config_name[:70] if disc.config_name else "", fuzzy_fmt
            )
    ...
    if all_corrupted_detailed:
        ...
        for ri, entry in enumerate(all_corrupted_detailed, 1):
            ws_corrupt.write(
                ri, 0, entry.get("file", entry.get("file_name", "")), cell_fmt
            )
